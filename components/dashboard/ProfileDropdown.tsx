...
const ProfileDropdown: React.FC<ProfileDropdownProps> = ({ onClose }) => {
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute right-0 mt-4 w-64 bg-darkblue2 border border-gray-700 rounded-lg shadow-xl z-50"
    >
      <div className="p-4 border-b border-gray-700">
        <div className="flex items-center space-x-3">
          {user.profilePictureUrl ? (
            <img src={user.profilePictureUrl} alt="Profile" className="h-14 w-14 rounded-full object-cover" />
          ) : (
        ...
        <Link to="/setting" onClick={handleLinkClick}>
          <button className="w-full mt-4 bg-accent hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200">
            Lihat Profil
          </button>
        </Link>
      </div>
      <nav className="p-2">
        ...
        <button
          ...
          className="flex items-center w-full px-3 py-2 text-sm text-gray-300 rounded-md hover:bg-danger hover:text-white"
        >
